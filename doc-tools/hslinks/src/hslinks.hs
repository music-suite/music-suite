...
runFilter inf outf args = do
  hSetEncoding inf utf8
  hSetEncoding outf utf8
  hGetContents inf >>= run args >>= hPutStr outf
...
run args input = do
  ...
  where
    ...
    substituteElements a = subRegex idExpr a "[`\\1`][x]"
...
--    in case whichModule (fmap snd sources) (wrapOp ident) of
--         Left e -> "[" ++ ident ++ "]: " ++ "\n<!-- Unknown: " ++ ident ++ " " ++ e ++ "-->\n"
--         Right modName ->
--           -- TODO
--           -- TODO This should be optional
--           let package = fromJust $ whichPackage sources modName
--            in ""
--                 ++ "["
--                 ++ ident
--                 ++ "]: "
...
--     whichModule' :: [ModuleName] -> Identifier -> Either String ModuleName
--     whichModule' modNames ident =
--       eitherMaybe ("No such identifier: " ++ ident) $
