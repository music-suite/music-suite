...
scatPad :: Duration -> [Score a] -> Score a
scatPad t xs = removeRests $ scat $ map (\x -> fmap Just x |> stretch t rest) xs

scatPaused :: [Duration] -> [Score a] -> Score a
scatPaused ts xs = removeRests $ scat $ zipWith (|>) (fmap (`stretch` rest) ts) (map (fmap Just) xs)
...
(<<>) x y = ucat [y,x]
spcat = scat . fmap pcat
pscat = pcat . fmap scat
...
simplifyPitches :: (HasPitches' a, PitchOf a ~ Pitch) => a -> a
...
renderFloater = ucat . fmap renderAlignedVoice . reverse . fmap (fmap $ (^.voice) . return) . getFloater
...
-- standardInstrumentRanges = scat $ map (\i -> addText (show i) $ set parts' (tutti i) $ onsetAndOffsetToMus (playableRange i))
...
--     onsetAndOffsetToMus x = let (m,n) = x^.from ambitus in pcat $ map fromPitch'' $ enumChromaticFromTo m n
--
...
showAmbitus a = let (m,n) = a^.from ambitus in pcat $ fmap (pure . colorBlue . fromPitch'') [m, n]
...
showPermutation p = stretchTo 1 $ scat . fmap ([c..] !!) $ indexPermutation p
...
renderPattern (Pattern ((unzip . fmap (^.from placed)) -> (origos,lungas))) s = pcat
...
ucat :: (Monoid a, Semigroup a, HasParts a a, PartOf a ~ Part) => [a] -> a
...
fromChords = join . fmap pcat . (fmap.fmap) fromPitch''
...
--
-- >>> filterOnsetOffsetDivisibleBy (1/4) $ compress 4 $ scat [c,d^*2,e::Score Pitch]
...
play2P :: [Hertz] -> IO ()
playS :: [Hertz] -> IO ()
playP = (\x -> openSuperCollider (x::Score (PartT Part {-Cents-}Double))) . ucat . fmap (pure . pure . toMidiP)
playS = (\x -> openSuperCollider (x::Score (PartT Part {-Cents-}Double))) . compress 12 . scat . fmap (pure . pure . toMidiP)
toMidiP = \f -> toDouble $ cents (f/440)/100 + 9
play2P xs = playUsingTim $ pcat $ map fromPitch'' $ map (^.from pitchHertz) $ xs
...
listRhythms'       = openLilypond' LyScoreFormat =<< (fmap ucat $ withStashR (\k -> (fmap (text (show k)) . get_) $ fst k) "^rh")
listMelodies'      = display =<< (fmap ucat $ withStashR (\k -> (fmap (text (show k)) . get_) $ fst k) "^mel")
listAccompaniments'= display =<< (fmap ucat $ withStashR (\k -> (fmap (text (show k)) . get_) $ fst k) "^accomp")
...
instance Inspectable [Music] where
  inspectableToMusic = ucat . fmap inspectableToMusic
...
renderMel xs = mcatMaybes $ pcat $ fmap renderAlignedVoice $ go xs
  where
